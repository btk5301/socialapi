...
async def test_create_post_expired_token(
    ...
    mocker,  # mocker can change the return values of certain functions
):
...
async def test_get_all_posts(async_client: AsyncClient, created_post: dict):
    ...
    # assert response.json() == [{**created_post, "likes": 0}] or
    assert created_post.items() <= response.json()[0].items()


# if lots of duplication where string changes and produces slightly different response, use parameterization
# pass certain values to a test and the test will run with those values
@pytest.mark.anyio
...
@pytest.mark.anyio
async def test_get_all_sort_likes(
    async_client: AsyncClient,
    logged_in_token: str,
):
    await create_post("Test Post 1", async_client, logged_in_token)
